function billPayment(){
		...
		var grid;
		var columnFilters = {};
        var data = document.getElementById("bilList").value
		console.log(data);
		var data1 = JSON.parse(data);
	    var paymentView=[];
	    for(var i=0; i<data1.length; i++){
	    paymentView.push(data1[i]['billno']);
	  ...
	  var check = document.querySelectorAll('input[type="checkbox"]:checked');
       ...
       function buttonFormatter(row, cell, value, columnDef, dataContext) {
	        ...
	        let b=dataContext.amt;
			//return '<input type="checkbox" value="'+a+','+b+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
			return '<form action="/viewbill" method="post" th:object="${checkbox}"><button type="submit" name="mail" value="'+a+','+b+'">Payable</button></form>';
