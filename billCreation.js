 ...
 */
 function billCreation(){
		...
		var grid;
		var columnFilters = {};
		var data = document.getElementById("billgenerate").value
		console.log(data);
			var data1 = JSON.parse(data);
     
     var medicineList=[];
     for(var i=0; i<data1.length; i++){
	medicineList.push(data1[i]['preno']);
     console.log(data1[i]['preno']);
	...
	var columnFilters = {};
		...
		let a=dataContext.preno;
		let b=dataContext.patid;
		return '<input type="checkbox" value="'+b+','+a+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
	}
    //var check= document.getElementById("checkBox");
   var check = document.querySelectorAll('input[type="checkbox"]:checked');
console.log(check);
function buttonFormatter(row, cell, value, columnDef, dataContext) {
			let a=dataContext.appointmentNo;
		   let b=dataContext.patid;
			//return '<input type="checkbox" value="'+a+','+b+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
			return '<form action="/billcreat" method="post" th:object="${checkbox}"><button type="submit" name="catogories" value="'+b+','+a+'">Fix Amount</button></form>';
