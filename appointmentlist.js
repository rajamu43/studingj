 ...
 */
 function appointmentList(){
	...
	appointmentList.push(data1[i]['patientid']);
     console.log(data1[i]['patientid']);
	...
	let a=dataContext.patientsid;
		let b=dataContext.regid;
		let c=dataContext.appdate;

		return '<input type="checkbox" value="'+b+','+c+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
	}
function dateFormatters(row, cell, value, columnDef, dataContext) {
...
console.log(check);
	function buttonFormatter(row, cell, value, columnDef, dataContext) {
		var today=new Date();
		console.log(today);
    var date= new Date(dataContext.appdate);
    console.log(date);
    if(date >= today){
			let a=dataContext.patientsid;
		let b=dataContext.regid;
		...
		let e=dataContext.diseaes;
			//return '<input type="checkbox" value="'+a+','+b+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
			return '<form action="/giveappoint" method="post" th:object="${check}"><button type="submit" name="doctorGender" value="'+b+','+c+','+e+','+d+','+a+'">Acceptance</button></form>';
		}
		else{
			let b=dataContext.regid;
		let c=dataContext.appdate;
        return '<form action="/giveappoints" method="post" th:object="${check}"><button type="submit" name="doctorGender" class="btn" value="'+b+','+c+'">Datechange</button></form>';
    }	
		}
    ...
    if(date <= today){
        ...
        return '<form action="/giveappoints" method="post" th:object="${check}"><button type="submit" name="doctorGender" value="'+b+'">Changedate</button></form>';
    }	
		}
		function dateFormatter(row,cell,Value,coloumnDef,dataContext){
    var today=new Date();
    var date= new Date(dataContext.value);
