function allPayments(){
		...
		var grid;
		var columnFilters = {};
        var data = document.getElementById("billView").value
		console.log(data);
		
	 var data1 = JSON.parse(data);
	 var payableList=[];
	    for(var i=0; i<data1.length; i++){
	     payableList.push(data1[i]['billsno']);
         console.log(data1[i]['billsno']);
     ...
     console.log(payableList);
	   ...
	   function checkboxFormatter(row, cell, value, columnDef, dataContext) {
		let a=dataContext.billsno;
		let b=dataContext.patientsid;
		let c=dataContext.dfees;
		let d=dataContext.mfees;
		let e=dataContext.amount;
		let f=dataContext.catogories;
		return '<input type="checkbox" value="'+a+','+b+','+c+','+d+','+e+','+f+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
	}
       ...
       function buttonFormatter(row, cell, value, columnDef, dataContext) {
		...
		let e=dataContext.amount;
		let f=dataContext.catogories;
		let g=dataContext.billdate;
			//return '<input type="checkbox" value="'+a+','+b+'" name="checkName" id="checkBox"' + (value ? 'checked="checked"' : '') + '/>';
			return '<form action="/billopen" method="post" th:object="${checkbox}"><button type="submit" name="mail" value="'+a+','+b+','+c+','+d+','+e+','+f+','+g+'">ViewBill</button></form>';
